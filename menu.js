...
const menu = {
  ...
  SendMoney: async (textArray, phoneNumber) => { 
    ...
    if (level == 1) {
      ...
      let sender = await User.findOne({ number: phoneNumber });
      ...
      if (!receiver) {
        ...
        sender.balance = senderBalance.toFixed(2);
        receiver.balance = receiverBalance.toFixed(2);
        ...
        let senderTransaction = new Transaction({
          ...
          user: sender._id,
          description: `Sent K${amount} to ${receiverName}`
        });
        let receiverTransaction = new Transaction({
          ...
          user: receiver._id,
          description: `Received K${amount} from ${senderName}`
        });
